mylabelling_fix <- function(X, root = NA, k = 40){
  ...
  U <- array(0, dim = c(ncol(X),ndim, nrow(X)))
  for (i in 1:n) {
  ...
  for (i in 1:n){
    Xii = X1 - matrix(rep(X1[i,],times=n),nrow = n,ncol = d,byrow = T)
    W = Xii %*% t(Xii)
    condir[i] = length(W[W>0])/(n*(n-1))
  }
  
  End = which(condir>0.9)
  ...
  S<-array(NA, dim = c(1, nrow(X), num_branch))
  ...
  for (i in 1:length(Branch_sort)){
    ...
    if (1){
      ...
      for (t in 1:num_branch){
        ...
        near[t] <- S[, ,t][b]
        T <- U[,,near[t]]
        dist2[t] <- norm((diag(d)-T %*% t(T)) %*% (t(X[branch,])-t(X[near[t],])),"2")
      }
    }
    ...
    # update tagent space
    I[,,branch] <- c(intersect(I[,,branch],S[, ,bb]),rep(NA,length(I[,,branch])-length(intersect(I[,,branch],S[, ,bb]))))
    Ii <- na.omit(I[,,branch])
    kt <- length(Ii)
